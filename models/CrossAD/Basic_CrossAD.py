...
class Basic_CrossAD(nn.Module):
    def __init__(self, configs):
        ...
        self.scale_ind_mask = self.ms_utils.scale_ind_mask(self.ms_p_lens).to(configs.device)
        self.next_scale_mask = self.ms_utils.next_scale_mask(self.ms_p_lens).to(configs.device)
        ...
        self.encoder = Encoder(
            layers=[
                EncoderLayer(
                    attention=AttentionLayer(ScaledDotProductAttention(attn_dropout=configs.attn_dropout),
        ...
        self.decoder = Decoder(
            layers=[
                DecoderLayer(
                    self_attention=AttentionLayer(ScaledDotProductAttention(attn_dropout=configs.attn_dropout),
                                                  d_model=configs.d_model, n_heads=configs.n_heads,
                                                  proj_dropout=configs.proj_dropout),
                    cross_attention=AttentionLayer(ScaledDotProductAttention(attn_dropout=configs.attn_dropout),
                                                   d_model=configs.d_model, n_heads=configs.n_heads,
                                                   proj_dropout=configs.proj_dropout),
                    d_model=configs.d_model,
                    d_ff=configs.d_ff,
    ...
    def _ms_anomaly_score(self, ms_x_dec, ms_gt):
        ms_score = F.l1_loss(ms_x_dec, ms_gt, reduction="none")  # score: [bs x ms_t x c]
    ...
    def infer(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ms_gt, ms_x_dec, query_latent_distances = self._forward(x_enc, x_mark_enc, x_dec,
                                                                x_mark_dec)  # [bs x ms_t x c]
